...
def parse_args():
    ...
    if not args.verbose:
        ...
        if args.verbose == 1:
            ...
            log_formater_line = "[%(asctime)s %(levelname)s %(module)s:%(lineno)d] %(message)s"
                    ...
                    try:
                        ...
                        for descr_entry in report.next_data():
                            ...
                            if descr_entry.value is None or d_typ in (TabBType.NUMERIC, TabBType.LONG, TabBType.DOUBLE):
                                ...
                                d_value = str(descr_entry.value)
                            if d_unit is None:
                                feature_properties["data_{:03d}".format(j)] = {"name": d_name, "value": d_value}
                            else:
                                feature_properties["data_{:03d}".format(j)] = {"name": d_name, "value": d_value, "unit": str(d_unit)}
                            j += 1
